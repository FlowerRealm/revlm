...
协议类型 `type` 与价格倍率 `price_multiplier` 是 ChannelGroup 的属性，channel 本身只保留
`id/name/status/priority/base_url/api_key`。每个渠道组只绑定一种协议类型：`/v1/models` 先用
token 找到渠道组，再返回该 `ChannelGroup.type` 插件自己的模型；OpenAI 与 Anthropic 可以同时
安装和启用。
...
| [数据面插件与 /v1 协议 hook](docs/reference/proxy-response-gateway.md) | 预加载替换、协议路由与流式用量链路 |
