    ...
    const rows = [
      {
        ...
        price: pricing.cache_read_input_usd_per_1m || '0',
      },
    ];
    // 仅当存在 1h 写入时才拆成两行，否则保持单行“缓存创建”（兼容历史数据）。
    if ((pricing.input_tokens_cache_creation_1h || 0) > 0) {
      rows.push({
        key: 'cache_creation_input_5m',
        label: '缓存创建·5m',
        tokens: pricing.input_tokens_cache_creation_5m || 0,
        price: pricing.cache_creation_input_usd_per_1m || '0',
      });
      rows.push({
        key: 'cache_creation_input_1h',
        label: '缓存创建·1h',
        tokens: pricing.input_tokens_cache_creation_1h || 0,
        price: pricing.cache_creation_1h_input_usd_per_1m || '0',
      });
    } else {
      rows.push({
