...
Channel::Channel(long long id, std::string name, bool status, int priority, std::string base_url, std::string api_key)
    : id(id)
    , name(std::move(name))
    , status(status)
    , priority(priority)
...
std::vector<Channel> ChannelStore::list_channels()
{
    ...
    const auto rows = sql_query_rows(db_, "SELECT id, name, status, priority, base_url, api_key FROM channels "
                                          "ORDER BY priority DESC, id DESC");
    t.commit();
    std::vector<Channel> out;
    for (const SqlResultRow &row : rows) {
        out.push_back(Channel(std::stoll(row[0].value_or("0")), row[1].value_or(""),
                              std::stoi(row[2].value_or("0")) != 0, std::stoi(row[3].value_or("0")),
                              row[4].value_or(""), row[5].value_or("")));
    }
    return out;
}
...
std::optional<Channel> ChannelStore::find_channel(long long id)
{
    ...
    return Channel(p->id, p->name, p->status, p->priority, p->base_url, p->api_key);
}
