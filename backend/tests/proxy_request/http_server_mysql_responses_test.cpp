...
int main()
{
    ...
    try {
        ...
        const long long user_id = user_store.create_user(revlm::CreateUserInput{
            "responses" + suffix + "@example.com", "responses" + suffix, revlm::hash_password("password"), "user" });
        ...
        const long long failover_token_id = token_store.create_user_token(user_id, std::nullopt, failover_raw_token);
