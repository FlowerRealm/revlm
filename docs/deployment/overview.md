...
- **API 网关**：Docker/Helm 镜像，入口是单个 `/revlm` 进程（初始化 schema、加载并注册插件，然后开始服务）
...
镜像内兼容插件放在只读的 `REVLM_SYSTEM_PLUGIN_DIR`（默认 `/usr/share/revlm/plugins`）。多架构镜像会在各自的构建平台编译并内置匹配的 OpenAI、Anthropic 包；它们可由 root 停用但不可从镜像删除。上传同 ID 包会覆盖镜像包；卸载覆盖包并重启后会自动恢复镜像版本。安装与卸载只标记状态，下一次冷启动才生效（卸载在那时才调用插件的清理入口并删包）；启用与停用即时改变注册表里的路由和模型目录，但新启用插件的数据库迁移同样等下一次冷启动。服务不会热加载模块。

插件不受 SDK 或能力白名单限制：宿主 `dlopen` 它并调用注册入口，插件随后直接使用核心的 C++ 数据面
类型。因此生产包必须与正在运行的镜像使用同一版本的 Revlm headers、编译器、标准库和
`librevlm_core`；不要把不同发行版的预编译 `.so` 混用。
