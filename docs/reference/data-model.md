...
`internal/store/migrations/0001_initial_schema.sql` 后的
...
- `updated_at` 不存在，Web session 不依赖用户行更新时间做失效判断。
...
- `id`: channel 主键。
- `type`: 上游类型，`1=openai_responses`、`2=openai_chat`、`4=anthropic`。
- `name`: 渠道名。
- `status`: 状态，`1=启用`、`0=禁用`。
...
模型目录不是数据库表。C++ 后端在 D011 中用代码常量提供模型列表、归属方、价格和缓存价格。
